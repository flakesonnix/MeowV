...
use anyhow::Result;
use protocol::{
    ClientMessage, PROTOCOL_VERSION, ResourceAvailabilityEntry,
    ResourceAvailabilityReport, ResourceAvailabilityStatus, ServerMessage, TrustedKey,
    decode_server_line, encode_line, build_signature_verification_plan,
};
use protocol::signature_engine::{
    execute_verification_plan, evaluate_signature_policy, SignaturePolicy,
};
use server::{
    ServerConfig, ServerSection, SharedState, run_with_listener_and_state,
};
use tokio::{
...
async fn live_unsigned_announcement_report_only_accepts() -> Result<()> {
    ...
    assert!(announcement.signature.is_none(), "server produces unsigned announcements");
    ...
    assert!(!report.all_valid(), "unsigned announcement should not be valid");
    ...
    assert!(report_text.contains("skipped"), "should report skipped: {report_text}");

    let resources = announcement.resources.iter().flat_map(|r| {
