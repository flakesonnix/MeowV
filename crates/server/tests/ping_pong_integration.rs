...
async fn handshake_then_ping_pong() -> Result<()> {
    ...
    loop {
        ...
        match msg {
            ServerMessage::Pong { sequence } => {
                assert_eq!(sequence, 1);
                break;
            }
            ServerMessage::ChatBroadcast { .. }
            | ServerMessage::EntitySnapshot { .. }
            | ServerMessage::ResourceAnnouncement(_) => { /* skip */ }
...
async fn ping_before_login_is_invalid() -> Result<()> {
    ...
    writer_half
        .write_all(encode_line(&ClientMessage::Ping { sequence: 123 })?.as_bytes())
        .await?;
